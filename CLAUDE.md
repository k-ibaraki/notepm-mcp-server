...
- 404 と 400 のメッセージは、呼び出し元が `not_found_message` / `bad_request_message` で差し替えられます。何が見つからなかったのかを知っているのは呼び出し元だけだからです。400 にも口を開けてあるのは、実 API が存在しない `page_code` に対して 404 ではなく 400（本文は `{"messages":["権限がありません"]}`）を返すためで、詳細取得だけがこの口を使います。NotePM が存在しないページと権限の無いページを区別しないので、こちらも断定せず両方の可能性を挙げます。404 の分岐は、返らないと確かめたわけではないので残してあります（Issue #31）
