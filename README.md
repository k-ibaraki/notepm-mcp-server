...
待ち時間が試行回数の分だけ積み上がるためです。再試行しない代わりに、他の通信の失敗と
同じ分類のエラーとしてすぐに返します（「エラーの分類」を参照）。
...
| パラメータを受け付けてもらえなかった、または対象のページに辿り着けない | 400 | 指定した値を見直す。詳細取得では、失敗した `page_code` を添えて返す |
| 対象の URL が見つからない | 404 | 詳細取得では、失敗した `page_code` を添えて返す |
| リクエスト制限を超えた | 429 | 時間をおいて再試行する |
| NotePM 側で処理に失敗した | 5xx | 一時的な障害の可能性があるため、時間をおいて再試行する |
| 上限の時間までに応答が返らない | - | 60 秒で打ち切る。時間をおいて再試行する |
| NotePM との通信が最後まで完了しない | - | ネットワークの状態を確認し、時間をおいて再試行する |
| 応答が JSON として読めない | 200 | メンテナンス中などで JSON 以外が返っている可能性がある |

存在しない `page_code` を詳細取得へ渡したとき、NotePM は 404 ではなく 400 を返します
（応答本文は `{"messages":["権限がありません"]}`）。存在しないページと権限の無いページを
NotePM が区別しないため、詳細取得の 400 では両方の可能性を挙げたうえで `page_code` を
添えて返します。表の 404 が指すのはチーム名の誤りのような URL 水準の失敗で、この分岐は
返らないと確かめたわけではないので残してあります。

リクエスト制限の具体的な上限値は、エラーメッセージには載せていません。NotePM の API
ドキュメントは「ユーザ毎に1分間に60リクエスト」としていますが、実応答の
`X-RateLimit-Limit` ヘッダは 120 を返します。どちらが実際の上限かをこちらでは断定
できないため、確かめられない数字を呼び出し側へ渡さない方針です。

429 と 5xx、接続の確立失敗、切られていた接続は「HTTP 接続の扱い」の再試行を尽くした
うえで、それでも直らなかったときにこの形で返ります。読み取りの時間切れ（30 秒）は
再試行しないため、1 回目でこの形になります。
...
| 水準 | 出力する内容 |
| --- | --- |
| WARNING | 入力検証や未知のツール名で弾いた呼び出し、再試行することにした一時的な失敗、NotePM とのやり取りで生じた失敗 |
...
この `httpx2` の 1 行には、URL の一部として検索語（クエリ文字列）が含まれます。上の表で
クエリを DEBUG に挙げているのはこのサーバー自身の出力の話で、検索語が記録に現れるのは
`-v` からです。検索語を残したくない場合は既定の水準のまま運用してください。
