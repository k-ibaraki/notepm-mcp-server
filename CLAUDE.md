...
- NotePM とのやり取りで生じた失敗は `NotePMError` の階層に分類します。ステータスの振り分けは `_raise_for_status()` にあり、401 / 403 は `NotePMAuthError`、400 は `NotePMBadRequestError`、404 は `NotePMNotFoundError`、429 は `NotePMRateLimitError`、5xx は `NotePMServerError` です。NotePM の API ドキュメントが挙げる 400 / 401 / 404 / 429 / 500 を土台に、HTTP の標準的な意味で読める範囲（403 の権限不足、500 番台の全体）まで広げてあり、そこから外れるステータスは意味を推測せず `NotePMAPIError` のままにします。JSON として読めない応答は `NotePMResponseError`、全体の上限時間での打ち切りは `NotePMTimeoutError`、接続や送受信の途中の失敗（`httpx2` の `TransportError` 全体。主な経路は、再試行を尽くした接続失敗と、再試行しない `ReadTimeout`）は `NotePMConnectionError` で、ステータス由来の失敗と分けています。`NotePMConnectionError` の文言に載せるのは例外の種別だけで、`httpx2` の文言はサーバー由来の受信データが混ざり得るため `_get()` が `%r` でログに残します（Issue #34）
- 404 のメッセージは呼び出し元が `not_found_message` で必ず渡し、400 のメッセージは `bad_request_message` で任意に差し替えます（省略すると一般的な文言）。何が見つからなかったのかを知っているのは呼び出し元だけだからです。400 にも口を開けてあるのは、実 API が存在しない `page_code` に対して 404 ではなく 400（本文は `{"messages":["権限がありません"]}`）を返すためで、詳細取得だけがこの口を使います。NotePM が存在しないページと権限の無いページを区別しないので、こちらも断定せず両方の可能性を挙げます。404 の分岐は、返らないと確かめたわけではないので残してあります（Issue #31）
...
- `page_code` は詳細取得 URL のパス要素へ直接埋め込むため、`PAGE_CODE_PATTERN` でパスの構造を変え得る文字（空白・制御文字と `/` `\` `.` `?` `#` `%` `:`）を拒みます。ここを緩めると、httpx2 の URL 正規化を介してページ詳細以外のエンドポイントへ到達できるようになります。長さも `PAGE_CODE_MAX_LENGTH` で縛ります。失敗した `page_code` はエラーメッセージへそのまま載るため、上限が無いと誤った値が届くたびにその全長が呼び出し側の文脈とログへ流れます
